...
Page({
  ...
  saveCart: function () {
    ...
    wx.getStorage({
      ...
      success: function (res) {
        ...
        app.api_util.trolley_insert(data, "正在操作", function success(res) {
          that.hideModal();
          wx.showToast({
            title: '添加成功',
            icon: 'success',
  ...
  getProducts: function (id) {
    var that = this;
    app.api_util.getProductInfo(id, "", function success(res) {
      if (res.errcode == 0) {
        ...
        var article = res.result.details;
  ...
  getcouponList: function () {
    ...
    app.api_util.getcouponlist({ memberId: memerId }, '', function success(res) {
      if (res.errcode == 0) {
        that.setData({
          list: res.result
        })
  ...
  insertcouponuser: function (e) {
    ...
    app.api_util.insertcouponuser(data, '', function success(res) {
      if (res.errcode == 0) {
  ...
  getsponsor: function () {
    var that = this;
    var memberId = Number(that.data.member.id);
    var activityId = Number(that.data.skList.id);
    var reGroupId = Number(that.data.skProduct.id);
    var sponData = {
      'memberId': memberId,
    ...
    app.api_util.getsponsor(sponData, '', function success(res) {
      that.setData({
        sponsor: res
      })
    }, function fail(res) {
  ...
  insertsponsor: function (e) {
    ...
    if (that.data.isStart == 1) {
      ...
      if (that.data.sponsor.errcode == 2) {
        ...
        if (that.data.sponsor.result.number==0){
  ...
  sponsorlist: function () {
    ...
    app.api_util.sponsorlist({ id: reGroupId, status: 1 }, '', function success(res) {
      if (res.errcode == 0) {
        var sp=[];
  ...
  onReady: function () {
    ...
    wx.getStorage({
      key: 'shopInfo',
      success: function (res) {
        var type_ = res.data.type;
        if (type_ == 2 && that.data.stype == 3) {
          that.getsponsor();
          that.sponsorlist();
        }
  ...
  onShareAppMessage: function (res) {
    ...
    return {
      ...
      path: "pages/mall/index/index?type=1&id=" + that.data.id + "&mid=" + that.data.member.id,
      success: function (res) {
