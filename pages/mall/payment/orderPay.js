...
Page({
        ...
        getData: function () {
                ...
                app.api_util.getusercouponlist(data, "加载中..", function sussess(res) {
                        if (res.errcode == 0) {
        ...
        integLabel: function (e) {
                ...
                if (value != 0) {
                        var money = (value / Number(that.data.shopInfo.integralDeduction)) * 100;
                        var total = (sum - money);
                        if (total < 0) {
                                total = 0
                        }
        ...
        toBuy: function (e) {
                ...
                var data = {
                        ...
                        "isCoupon": that.data.shopInfo.isCoupon,
                        "createUserId": app.globalData.createUserId
                };
                data.couponValue = 0;
                if (that.data.shopInfo.isCoupon == 1) {
        ...
        comPay: function (data, openId) {
                var that = this;
                app.api_util.order_insert(data, "", function success(res) {
                        if (res.errcode == 0) {
                                var outTradeNo = res.result.id;
                                if (that.data.reCouponId != '') {
                                        app.api_util.updatecouponuser(that.data.reCouponId, '', function succes(res) { }, function fail(res) { });
                                }
                                ...
                                if (res.result.totalPrices != 0) {
                                        app.api_util.wechat_pay('商品支付', res.result.id, res.result.totalPrices, openId, 1);
                                        that.updateMember();
                                } else {
                                        app.api_util.order_pendingshipment(outTradeNo, '正在提交中', function success(res) {
                                                if (res.errcode == 0) {
                                                        wx.redirectTo({
        ...
        skecPay: function (data, openId) {
                ...
                app.api_util.insertseckill(id, data, "", function success(res) {
                        if (res.errcode == 0) {
                                console.log(res.result)
                                var outTradeNo = res.result.id;
                                app.api_util.wechat_pay('商品支付', res.result.id, res.result.totalPrices, openId, 2);
        ...
        sponPay: function (data, openId) {
                ...
                if (that.data.sponsor == '') {
                        ...
                        app.api_util.insertsponsor(sponData, '', function success(res) {
                                if (res.errcode == 0) {
        ...
        insertgroup: function (sponid, data, openId) {
                var that = this;
                app.api_util.insertgroup(sponid, data, "", function success(res) {
                        if (res.errcode == 0) {
                                if (res.result != '') {
                                        var outTradeNo = res.result.result.id;
                                        console.log(res.result)
                                        app.api_util.wechat_pay('商品支付', outTradeNo, res.result.result.totalPrices, openId, 3);
                                        wx.setStorage({
                                                key: 'grouporder',
                                                data: res.result,
                                        })
