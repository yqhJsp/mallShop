...
Page({
  ...
  onLoad: function (options) {
    ...
    app.api_util.order_one(id, '', function success(res) {
      console.log(res);
      if (res.errcode == 0) {
        var list = res.result;
        if (list != '') {
  ...
  modalOk: function () {
    ...
    app.api_util.order_close(oid, '', function success(res) {
      if (res.errcode == 0) {
        if (order.type == 3) {
          that.deletesponsor();
        }
  ...
  goPay: function () {
    ...
    app.api_util.wechat_pay('商品支付', order.id, order.totalPrices, member.openId,type);
