...
Page({
  ...
  onLoad: function (options) {
    ...
    app.globalData.member = that.data.member;
  },
  //获取审核状态
  getdistributoraudit: function () {
    var that = this;
    var member = app.globalData.member;
    app.api_util.getdistributoraudit({ memberId: member.id }, '', function success(res) {
      if (res.errcode == 0) {
        ...
        if (that.data.distrib.status==1){
          member.distributorRole=1;
          that.setData({
            member: member
          })
          app.globalData.member.distributorRole =1;
        }
  ...
  getBrokeraGeapplylist: function () {
    ...
    var member = app.globalData.member;
    app.api_util.brokerageapplylist({ size: size, number: number }, "", function success(res) {
      if (res.errcode == 0) {
  ...
  getBrokeraloglist: function () {
    ...
    var size = that.data.size;
    var number = that.data.number;
    app.api_util.brokerageloglist({ size: size, number: number }, "", function success(res) {
      if (res.errcode == 0) {
        if (res.result.content.length != 0 && res.result.content != '') {
          that.setData({
