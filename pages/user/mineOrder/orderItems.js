...
Page({
  ...
  chooseReson: function (options) {
    ...
    console.log(that.data.resonId+"resonId")
  },
  /*弹窗确认按钮事件*/
  modalOk: function () {
    var that = this;
    ...
    app.api_util.order_close(orders.id, '', function success(res) {
      if (res.errcode == 0) {
        that.getData();
  ...
  deletesponsor: function (id) {
    app.api_util.deletesponsor(id, '', function success(res) {
      console.log('取消开团成功');
      that.getData();
    }, function fail(res) {
    })
  ...
  /*跳转订单详情页*/
  goOrderDetail: function (options) {
    var that = this;
    var id = options.currentTarget.dataset.id;
    var safeguard = options.currentTarget.dataset.type;
   ...
   * 打开快递物流页面
   */
  openExpressage: function (options) {
    var that = this;
    var code = options.currentTarget.dataset.code;
    var number = options.currentTarget.dataset.number;
    wx.navigateTo({
  ...
  onLoad: function (options) {
    ...
    that.setData({
     currentNavtab: tab,
      ...
      appid: app.globalData.shopInfo.appid,
  ...
  getData: function (msg) {
    ...
    app.api_util.order_list(orderParem, "加载中", function sussess(res) {
  ...
  insertintegral: function (orderId, integLabel) {
    ...
    var integralConvert = app.globalData.shopInfo.integralConvert;
    ...
    console.log(integralConvert)
        var data = {
          ...
          app.api_util.insertintegrallog(data, '', function success(res) {
            if (res.errcode == 0) {
              that.updateMember();
            }
          }, function fail(res) {
  ...
  goPay: function (e) {
    ...
    var totalPrices = e.currentTarget.dataset.total;
    var type = e.currentTarget.dataset.type;
    wx.showLoading({
      title: '结算中',
    })
    app.api_util.wechat_pay('商品支付', id, totalPrices, member.openId, type);
  },
  /*确认收货*/
  comfirmOrder: function (options) {
    var that = this;
    var oid = options.currentTarget.id;
    var sum = (options.currentTarget.dataset.sum)/100;
    console.log(sum)
    wx.showModal({
      title: '确认收货',
      content: '是否确认收货？',
      success: function (res) {
        if (res.confirm) {
          app.api_util.order_receipt(oid, '', function success(res) {
            if (res.errcode == 0) {
              if (that.data.shopInfo.isIntegral == 1) {
  ...
  /*评价*/
  evluateOrder: function (options) {
    var that = this;
    var pid = options.currentTarget.id;
    var oid = options.currentTarget.dataset.id;
  ...
  subscribelist: function () {
    var that = this;
    var memberId = app.globalData.member.id;
    app.api_util.subscribelist({ memberId: app.globalData.member.id}, '', function success(res) {
          if (res.errcode == 0) {
            var list = res.result;
  ...
  cancelSub: function (e) {
    ...
    wx.showModal({
      ...
      success: function (res) {
        if (res.confirm) {
          app.api_util.updatesubscribe(id, '', function success(res) {
            if (res.errcode == 0) {
              that.subscribelist();
            }
